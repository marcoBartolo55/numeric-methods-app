...
import com.numeric.methods.logic.RungeKutta4;
import com.numeric.methods.logic.RungeKutta4.ResultRow;
...
public class rungeKutta4Controller {
    ...
    @FXML private TableColumn<ResultRow, String> k4Column;
    ...
    public void initialize() {
        ...
        k3Column.setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getK3()));
        k4Column.setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getK4()));
        resultTable.setItems(tableData);

    }

    @FXML
    private void calculateRungeKutta4() {
        tableData.clear();
        ...
        try {
            ...
            RungeKutta4 rungeKutta4 = new RungeKutta4(function, x0, y0, xf, h);
            showInfoAlert("Cálculo del método de Runge-Kutta 4 completado exitosamente.");
            List<ResultRow> results = rungeKutta4.generateTableData();
            tableData.addAll(results);

        } catch (NumberFormatException e) {
            
            showErrorAlert("Por favor, asegúrese de que los campos numéricos contienen valores válidos.");

        } catch (Exception e) {
            
            showErrorAlert("Ocurrió un error al calcular el método de Runge-Kutta 4.");
