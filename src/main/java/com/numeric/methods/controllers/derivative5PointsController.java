...
import com.numeric.methods.logic.Derivative5Points;
...
public class derivative5PointsController {
    ...
    // Componentes de la tabla (5 puntos)
    @FXML private TableView<DerivativeRow> resultTable;
    @FXML private TableColumn<DerivativeRow, String> rowLabelColumn, point1Column, point2Column, point3Column, point4Column, point5Column;

    private final ObservableList<DerivativeRow> tableData = FXCollections.observableArrayList();

    @FXML
    private void initialize() {
        // Configurar columnas de la tabla mapeando al índice del arreglo interno (0 a 4)
        ...
        point3Column.setCellValueFactory(d -> d.getValue().puntoProperty(2));
        point4Column.setCellValueFactory(d -> d.getValue().puntoProperty(3));
        point5Column.setCellValueFactory(d -> d.getValue().puntoProperty(4));
        
        resultTable.setItems(tableData);

        // Lógica dinámica para mostrar/ocultar paneles
    ...
    private void calculateDerivative() {
        try {
            ...
            if ("h igualmente espaciada".equals(selectedMethod)) {
                ...
                // Magia: Generamos exactamente 5 puntos usando el método reciclado
                points = Derivative2Points.generatePointsEquallySpaced(func, x0, h, 5);
                ...
                if (points.size() != 5) {
                    throw new Exception("Se requieren exactamente 5 puntos (10 valores: x0, y0, ... x4, y4).");
                }
            }
    ...
    private void fillTable(List<Point> points) {
        tableData.clear();
        DerivativeRow rowX = new DerivativeRow("x", 5);
        DerivativeRow rowY = new DerivativeRow("f(x)", 5);
        DerivativeRow rowD = new DerivativeRow("f'(x)", 5);
        ...
        for (int i = 0; i < points.size(); i++) {
            ...
            // Calcular la derivada en el punto i usando la clase lógica de 5 puntos
            double der = Derivative5Points.calculateDerivative(points, points.get(i).getX());
